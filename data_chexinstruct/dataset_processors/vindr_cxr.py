...
class VinDRCXRProcessor(BaseProcessor):
    ...
    def create_abnormality_grounding(self, qbins=1000):
        dataset = {"train": [], "val": [], "test": [], "dataset_name": "[Abnormality Grounding] [VinDr-CXR]"}
        ...
        for split, split_data, meta_data in zip(
            ...
            for group_idx, group in split_data.groupby("image_id"):
                ...
                for class_name in set([region[0] for region in regions]):
                    # positive
                    unique_id = f"[Abnormality Grounding] [VinDr-CXR] [{group_idx}]"
                    study_id = f'[vindr-cxr] [{group_idx}]'
                    data_source = "VinDr-CXR"
                    ...
                    # negative
                    unique_id = f"[Abnormality Grounding] [VinDr-CXR] [{group_idx} - negative]"
                    study_id = f'[vindr-cxr] [{group_idx}]'
                    data_source = "VinDr-CXR"
                    task_name = "Abnormality Grounding"
