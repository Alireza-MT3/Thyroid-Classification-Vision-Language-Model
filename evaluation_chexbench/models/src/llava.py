...
class LlavaLlamaModel(LlamaModel):
    ...
    def __init__(self, config: LlamaConfig, mm_vision_tower=None, mm_hidden_size=None):
        ...
        if hasattr(config, "mm_vision_tower"):
            ...
            if "BiomedCLIP" in config.mm_vision_tower or "biomed_clip" in config.mm_vision_tower:
                ...
                self.vision_tower = [
                    model.visual.trunk]  # Please refer: https://github.com/mlfoundations/open_clip/blob/main/src/open_clip/timm_model.py#LL60C18-L60C18
    ...
    def extract_visual_features(self, vision_tower, images):
        ...
        if "BiomedCLIP" in self.vision_tower_name or "biomed_clip" in self.vision_tower_name:
            image_forward_outs = vision_tower.get_intermediate_layers(images,
                                                                      n=3)  # take last n blocks if n is an int, if in is a sequence, select by matching indices
        ...
        if vision_tower is not None and (input_ids.shape[1] != 1 or self.training) and images is not None:
            ...
            with torch.no_grad():
                if type(images) is list:
                    ...
                    image_features = []
                    for image in images:
                        image_feature, dummy_image_features = self.extract_visual_features(vision_tower,
                                                                                           image.unsqueeze(0))
